...
| Messaging runtime | Supported for local/mock plus native Kafka/NATS publish and consume/observe dispatch | Messaging provider contract, runtime registry, native transport, CLI preflight, and evidence tests | Add message cleanup and owner broker-backed pilot validation. |
...
Current framework verification support is narrower than the pilot target: REST and native descriptor-driven gRPC unary calls are supported, local/mock messaging is supported, native Kafka/NATS publish and consume/observe dispatch is supported, JDBC DB fixture is supported, local/mock plus native K8s/VM bounded deployment readiness is supported, file/batch is supported, and approved command-runner external runner is supported as an escape hatch. Message cleanup, deeper K8s/VM probes, richer assertion/oracle providers, and real pilot environment evidence require separate implementation slices before they can satisfy full pilot acceptance.
