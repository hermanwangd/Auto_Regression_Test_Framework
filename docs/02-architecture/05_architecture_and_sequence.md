...
| `messaging/kafka` and `messaging/nats` | Supported for native publish, consume/observe, and bounded cleanup drain | `MessagingProvider` plus `DefaultMessagingTransport` | bootstrap/server/connection ref, topic or subject ref, supported action mode, payload binding for publish, cleanup strategy and positive max count for cleanup, positive `timeout_seconds`, `outputs.actual_output_ref`, correlation id when required | Framework provider-family and injectable transport tests; pilot broker evidence remains pending. |
