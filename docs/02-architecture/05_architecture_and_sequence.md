...
| `messaging/local` and `messaging/mock` | Supported for local/mock evidence | `MessagingProvider` | topic/subject/stream/endpoint ref, supported action mode, payload binding for publish, positive `timeout_seconds`, `outputs.actual_output_ref`, correlation id when required | Framework provider-family tests. |
...
| `messaging/kafka`, `messaging/nats`, native K8s, native VM | Supported for first native slices | Messaging and deployment readiness providers | Provider-specific connection refs, action mode support, bounded timeout, actual output refs, evidence mapping, failure semantics | Message cleanup, direct K8s API/pod logs, SSH/WinRM, and pilot environment evidence remain pending. |
