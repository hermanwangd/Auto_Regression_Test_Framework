...
- External runner entries are `contract_only` providers in v0.2.7. They require a later safety-approved runtime slice before release execution.
...
| `grpc_client` | PR-008B unary provider capability | Framework executes checked-in local/CI unary gRPC samples against generated `grpc_mock` `target_uri`; broader downstream gRPC endpoint evidence remains pilot work | target binding key, unary operation service and method, descriptor/message refs, positive timeout, output refs | Contract validation, dry-run planning, provider capability execution, happy/failure/boundary sample coverage, and `grpc_request_response` evidence. |
...
| `kafka` | P1 client provider target | Client-side Kafka provider only; connects to Env_Profile-supplied broker bindings and does not provision broker/Testcontainer/RU runtime in this slice | `bootstrap_servers`, `topic`, `consumer_group`, `kafka_publish`, `kafka_observe`, `kafka_payload_match`, timeout, poll interval, output refs | Contract validation, dry-run planning, mocked client tests, optional profile-gated native broker execution, and messaging evidence. Pilot broker evidence remains future. |
...
Mock-service lifecycle is suite/batch scoped. Framework-owned mock services such as WireMock HTTP, SOAP mock, or gRPC mock may be provisioned before RU readiness and reused by multiple DSL test cases with reset operations between tests. DSL `setup` may load stubs or seed state through Provider Contract operations, but it must not define RU startup, RU restart, mock-server process startup, or endpoint binding generation. Local and CI profiles may optionally launch an RU after dependency/mock binding outputs exist; SIT and preprod normally use predeployed RUs and readiness evidence. RU configuration consumption remains outside the core DSL and is handled by the environment owner, CI/local launcher, or a future explicit provider.
...
- Provider public-interface verification for request/response, messaging, DB fixture, deployment readiness, file/batch, and safety-approved command-capable provider behavior using local/mock or injectable fixtures.
