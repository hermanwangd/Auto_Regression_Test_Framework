...
| `messaging/kafka`, `messaging/nats`, native K8s, native VM | Supported for first native slices | Messaging and deployment readiness providers | Provider-specific connection refs, action mode support including bounded messaging cleanup drain, bounded timeout, bounded K8s `kubectl` and direct API readiness, bounded K8s pod log capture, bounded VM SSH/WinRM command probes, actual output refs, evidence mapping, failure semantics | Persistent broker purge if needed and pilot environment evidence remain pending. |
