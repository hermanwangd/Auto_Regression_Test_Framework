...
| External runner bridge | Contract validation, approved `command_runner` dispatch, approval metadata checks, bounded timeout checks, unsafe output path blocking, built-in provider alternative blocking, and evidence-map recording exist as an escape hatch | Governed escape hatch for JUnit, Newman, Robot, legacy binaries, or custom harnesses only when built-in providers cannot cover the boundary | Add richer mapped-artifact validation before broad use |
