...
| REST | Supported through configurable HTTP request/response provider with payload binding, evidence capture, JSON path equality assertions, and dry-run blocking for unsupported actions or unresolved payload bindings | Native request/response provider | Add response status helpers plus schema and contract checks when pilot requires them |
...
| Oracle/assertion | File diff, expected-result artifact path, JSON path equality assertions, and DB row count assertions | JSON path, schema, contract, DB row, absence, tolerance | Add schema, contract, absence, and tolerance assertion providers incrementally |
