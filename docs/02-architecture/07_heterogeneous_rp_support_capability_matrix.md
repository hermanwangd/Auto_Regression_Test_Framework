...
| Execution adapter | Provider runtime registry dispatches file/batch shell, REST, native descriptor-driven gRPC unary request/response, local/mock messaging, native Kafka/NATS publish, consume/observe, and cleanup messaging, JDBC DB fixture, local/mock plus native K8s/VM deployment readiness, bounded K8s pod log capture, and approved command-runner escape hatch paths | Provider-registry-dispatched REST/gRPC, Kafka/NATS, DB, deployment readiness, and shell/file providers | Add direct kube API, VM SSH/WinRM, and real pilot environment validation |
...
| NATS | Native `messaging/nats` publish, consume/observe, and cleanup runtime is supported through NATS core protocol dispatch with `server_ref`, `subject_ref`, mode, payload binding for publish, optional min/expected count for observe, `cleanup_strategy: drain`, positive `max_count`, JSON serialization gate, timeout, correlation, output ref, and evidence. | Publish/request/reply/subscribe where pilot needs it plus bounded drain cleanup | Add request/reply and pilot evidence against owner-provided NATS environment. |
...
| K8s readiness | Native `deployment_readiness/k8s` runtime supports bounded `kubectl` rollout/pod readiness probes and bounded pod log capture with kube context, namespace, deployment or selector, version ref, timeout, output ref, log tail bound, and evidence. Local/mock `file_exists` readiness remains supported. | Kube context, namespace, deployment, service, pod/log readiness provider | Add direct kube API and real pilot cluster evidence when owner environment exists. |
