...
| Kafka | Native `messaging/kafka` is not implemented and must block as unsupported; local/mock messaging contracts can exercise Kafka-like payload evidence, timeout, output ref, correlation, and unsupported-configuration blocking | Publish, consume, observe, correlate | Needs native Kafka client provider, topic refs, serialization, timeout, correlation, observation rule, and evidence rules |
| NATS | Native `messaging/nats` is not implemented and must block as unsupported; local/mock messaging contracts can exercise NATS-like payload evidence, timeout, output ref, correlation, and unsupported-configuration blocking | Publish/request/reply/subscribe where pilot needs it | Needs native NATS client provider, subject refs, correlation, timeout, observation rule, and evidence rules |
...
This example is a target-state RP contract sketch because it includes native Kafka observation. In current framework verification, `provider_type: kafka` is expected to block as unsupported; use `provider_type: local` or `mock` for local/mock messaging evidence until the native Kafka provider slice is implemented and verified.
