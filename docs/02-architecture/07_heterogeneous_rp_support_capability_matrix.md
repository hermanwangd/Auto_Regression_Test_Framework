...
| External runner bridge | Contract validation, approved `command_runner` dispatch, approval metadata checks, bounded timeout checks, and evidence-map recording exist as an escape hatch | Governed escape hatch for JUnit, Newman, Robot, legacy binaries, or custom harnesses only when built-in providers cannot cover the boundary | Add stronger built-in-provider alternative detection and richer mapped-artifact validation before broad use |
