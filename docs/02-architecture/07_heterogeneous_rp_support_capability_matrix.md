...
- Provider contracts bind logical DSL actions to concrete technology behavior such as REST, gRPC, Kafka, NATS, DB, K8s, VM, or an external runner.
- Providers normalize execution, assertion, cleanup, and evidence results back into the common RP evidence model.

Java, Go, C++, VB.NET, and other languages should normally be invisible to the DSL. Language matters only when a build, command, artifact, or external runner contract needs it.
...
- Legacy or specialized execution: external runner bridge.
...
| Execution adapter | Local shell command adapter | Provider-dispatched REST/gRPC, Kafka/NATS, DB, external runner, and shell providers | Current execution engine uses one shell-style adapter path |
...
| Orbix | Not implemented | External runner bridge first; native provider later if reusable | Avoid one-off RP-specific provider code |
...
| External runner bridge | Not implemented | Standard wrapper for JUnit, Postman/Newman, Robot, custom scripts, legacy binaries | Needs runner contract and evidence mapping |
| Provider registry | Structural resolver exists | Config-driven provider registry with default, RP, and RU precedence | Current resolver is minimal and not multi-RU capable |
...
- Target: required for the heterogeneous pilot but not yet implemented.
...
| External runner provider | Delegate execution to existing tools or legacy runtimes | JUnit, Newman, Robot, shell, C++ or VB.NET harness | Command or container ref, inputs, outputs, success codes, evidence artifact map |
...
Provider configuration must reference secrets and environment resources by name or secret ref. It must not inline credentials, production data, or destructive commands without explicit policy approval.
...
- Configure an existing provider before adding new provider code.
- Add provider code only when the behavior is reusable across RPs or cannot be safely represented by existing contracts.
...
- External runner bridge is acceptable for legacy or specialized systems when standard evidence can still be produced.
...
The next implementation slice should not start by coding REST, Kafka, K8s, and VM providers all at once. It should first introduce a provider dispatch contract that supports multiple RU entries, provider family metadata, capability readiness, and standardized evidence output. After that, add provider families in pilot order:
...
5. External runner provider for legacy or specialized execution.
