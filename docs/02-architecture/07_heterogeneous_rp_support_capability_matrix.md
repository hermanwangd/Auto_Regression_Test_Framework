...
| REST | Supported through configurable HTTP request/response provider with payload binding, evidence capture, HTTP/status field checks, JSON path equality and absence assertions, numeric tolerance assertions, and dry-run blocking for unsupported actions or unresolved payload bindings | Native request/response provider | Add schema and contract checks when pilot requires them |
...
| Oracle/assertion | File diff, expected-result artifact path, HTTP/status field checks, JSON path equality and absence assertions, numeric tolerance assertions, multi-assertion aggregation, and DB row count assertions | JSON path, schema, contract, DB row, absence, tolerance | Add schema and contract assertion providers incrementally |
