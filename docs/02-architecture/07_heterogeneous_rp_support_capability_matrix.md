...
| Execution adapter | Provider runtime registry dispatches file/batch shell, REST, native descriptor-driven gRPC unary request/response, local/mock messaging, native Kafka/NATS publish messaging, JDBC DB fixture, local/mock plus native K8s/VM deployment readiness, and approved command-runner escape hatch paths | Provider-registry-dispatched REST/gRPC, Kafka/NATS, DB, deployment readiness, and shell/file providers | Add Kafka/NATS consume/observe and richer K8s/VM readiness as reusable runtimes |
...
| Kafka | Native `messaging/kafka` publish runtime is supported through Kafka client dispatch with `bootstrap_servers_ref`, `topic_ref`, payload binding, JSON serialization gate, timeout, correlation, output ref, and evidence. | Publish, consume, observe, correlate | Add broker-backed consume/observe, message cleanup, and pilot evidence against owner-provided Kafka environment. |
| NATS | Native `messaging/nats` publish runtime is supported through NATS core protocol dispatch with `server_ref`, `subject_ref`, payload binding, JSON serialization gate, timeout, correlation, output ref, and evidence. | Publish/request/reply/subscribe where pilot needs it | Add request/reply or subscribe/observe, message cleanup, and pilot evidence against owner-provided NATS environment. |
...
This example is a target-state RP contract sketch because it includes native Kafka observation. Current framework verification supports native Kafka publish dispatch and blocks incomplete Kafka contracts with owner-actionable missing-field gaps. Kafka consume/observe evidence still requires a later provider slice or real pilot environment validation.
