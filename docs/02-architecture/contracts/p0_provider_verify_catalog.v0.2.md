...
| SOAP / Mock | `soap_mock`, `load_soap_stub`, `soap_request_received`, SOAPAction/header matching, XPath request matching, XML response, request journal, server log, assertion evidence | PR-008A executable WireMock-backed mock capability through `samples/provider_capability/soap_mock/` for local/CI happy, failure, and boundary paths. This is local/CI mock evidence only. |
