...
Env_Profiles own the dependency provisioning policy and provider binding keys for local and CI. For Testcontainers-backed or embedded dependencies, the Env_Profile declares allowed provisioners, dependency type, startup/readiness policy, cleanup scope, provisioner outputs, and provider bindings. Generated values such as JDBC URLs, broker bootstrap servers, mapped ports, service base URLs, or topic refs may be referenced through `generated_ref` only when the producing Provider Contract declares a matching `bindable_outputs` entry or the Env_Profile dependency provisioner declares the generated output.
...
- `generated_ref` values must target a producing Provider Contract `bindable_outputs` entry or a declared Env_Profile dependency provisioner output.
