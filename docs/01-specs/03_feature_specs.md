...
| Deployment readiness | Local/mock plus native K8s/VM bounded readiness evidence with deployed version ref, timeout, output ref, K8s pod log tail bound when configured, and bounded probe behavior. | Owner-provided pilot K8s/VM validation remains required for release acceptance. |
