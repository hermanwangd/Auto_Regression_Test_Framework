...
|---|---|---|---|---|
| EVD-000 | Framework verification reports, sample Product Repo fixture run output, packaged CLI smoke output, and CI command logs | Platform / QA | AC-010 proof that Maven verification validates the framework and does not claim downstream RP release evidence | `target/surefire-reports/`, `target/failsafe-reports/`, packaged CLI output, and CI artifacts |
