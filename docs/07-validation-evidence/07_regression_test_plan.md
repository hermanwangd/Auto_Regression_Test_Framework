...
| Response assertion provider | `ExecutionEngineTest` response assertion cases and `RequestResponseProviderTest` HTTP evidence cases | Supported for HTTP/status field checks, JSON path equality and absence assertions, numeric tolerance assertions, multi-assertion aggregation, and durable assertion evidence over provider actual-output JSON/YAML plus request-response evidence | Add schema and contract assertions when selected by the pilot. |
