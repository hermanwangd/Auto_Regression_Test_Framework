...
- `regress report` supports suite-mode with `--result <generated_result_json>`. It supports `--format text|yaml` and prints stable review keys. `--format json` must return usage error exit `2`.
