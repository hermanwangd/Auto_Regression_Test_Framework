...
| Response assertion provider | `ExecutionEngineTest` JSON path assertion cases | Supported for JSON path equality assertions over provider actual-output JSON/YAML with inline expected values and durable assertion evidence | Add response status helpers, schema, contract, tolerance, and absence assertions when selected by the pilot. |
