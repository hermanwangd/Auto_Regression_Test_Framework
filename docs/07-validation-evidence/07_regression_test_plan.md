...
| Messaging | Resolve publish/consume/observe/cleanup contract with topic or subject ref, payload binding for publish, correlation id when required, timeout, observation rule, cleanup strategy, and max cleanup count | Missing topic/subject ref, missing publish payload binding, missing cleanup strategy/count, missing correlation id where required, unsupported mode, or unsupported serialization blocks before dispatch | Provider family, topic/subject ref, action mode, correlation id, observed message/event ref, cleanup result |
...
| Deployment readiness | Resolve K8s and VM readiness contracts, version/deployment refs, readiness probes, and bounded K8s pod log capture | Missing deployment ref, pod selector/log tail bound, readiness probe, environment ref, or provider capability blocks affected RU before execution | Deployment provider family, readiness status, deployment/version ref, affected RU, captured pod log ref when configured |
