...
- Use sample DSL v0.2 tests, suite manifest, Execution Profile, Provider Instance, Provider Contract, Environment Binding, expected results, and traceability map to validate generic discovery, binding, provider execution, verification, result JSON, batch evidence, run evidence, and coverage report generation.
- Verify provider capability registry, Provider Contract schema validation, Provider Instance validation, Environment Binding validation, verify plugin contracts, and provider type behavior required by the selected heterogeneous pilot without treating mock or sample provider evidence as downstream RP release evidence.
...
| Packaged CLI smoke verification | Spring Boot jar CLI entrypoint | `java -jar target/spec-driven-auto-regression-0.1.0-SNAPSHOT.jar validate --root . --rp-id RP-FWK-SAMPLE --env ci` | Sample Product Repo fixture or current repository fixture | CLI output and exit code. |
...
Provider public-interface contract verification must prove that local and CI profiles can replace external service, DB, messaging, K8s, and VM dependencies with explicit mock, WireMock HTTP mock, stub, ephemeral, Testcontainers-backed, fake-topic, embedded-broker, disposable-schema, or generated-data bindings. It must validate dependency provisioning policy fields such as provisioner, dependency type, image/catalog ref, startup timeout, readiness check, cleanup scope, and output binding keys. Track A limits this to contract validation, sample parsing, and dry-run planning; provider behavior is verified in later runtime tracks. It must not require real SIT/UAT endpoints, real K8s clusters, real VMs, production data, or committed secrets. It must also prove that SIT/preprod release evidence cannot silently use mock substitution. External runner verification is limited to contract validation and provider safety approved escape-hatch behavior unless an explicit runner implementation slice is selected later. External runner approval is provider safety approval, not release approval.
...
- Generated suite manifest, run plan, Execution Profile, Provider Instance, Provider Contract, Environment Binding, target dependency, and missing-field handling.
- DSL v0.2 schema validation for identity, status, revision, tags, `source_refs`, optional `labels`, compatible profiles, targets with `provider_id` and `profile`, operation parameters with `ref` and `bind_as`, setup, execute, cleanup, expected results, verify, evidence, and runtime, plus execution lifecycle status checks and unsupported capability blocking.
...
- Unsupported provider types, missing Provider Contracts, missing Provider Instances, missing Environment Bindings, invalid `bind_as`, missing binding keys, missing cleanup policy, and missing readiness evidence fail before provider execution.
...
- Generated `suite_manifest.yaml`, `run_plan.yaml`, Execution Profiles, Provider Instances, Environment Bindings, Provider Contracts, and `traceability_map.yaml`.
...
|---|---|---|---|
| FWK-IT-001 | AC-001, AC-002, AC-004, AC-005, AC-009, AC-012, AC-013, AC-015, AC-017 | Happy path generated artifact validation, `run`, and `report` complete without SIT/UAT deployment | Valid DSL targets resolve through provider_id/profile, selected Execution Profile, Provider Instance, Provider Contract, Environment Binding, fixture setup/cleanup, shell provider execution, durable batch/run evidence, result JSON, suite selection, 100% sample coverage, and sample evidence boundary. |
| FWK-IT-003 | AC-001, AC-002, AC-005, AC-016 | Provider resolution failure | Missing Provider Instance, missing Provider Contract, missing Environment Binding, missing binding key, invalid `bind_as`, missing output ref, unsupported provider_type, unsupported runtime_mode, prohibited mock substitution, or ambiguous provider selection blocks before provider execution and writes blocked run evidence. |
...
| FWK-IT-010 | AC-002, AC-004, AC-015, AC-016 | Provider public-interface dry-run | Local/CI mock, stub, ephemeral, Testcontainers-backed, fake-topic, embedded-broker, disposable-schema, and native Provider Contracts and Provider Instances for selected provider types resolve through the capability registry with provider_id, provider_type, profile, runtime_mode, dependency provisioner, registry status, supported execution modes, evidence outputs, contract path, instance path, binding key status, and resolved execution plan. Missing, ambiguous, unsupported, invalid `bind_as`, missing output ref, missing binding key, unsupported runtime_mode, missing Testcontainer image/catalog ref, missing readiness check, missing output binding key, prohibited SIT/preprod mock substitution, or unapproved escape-hatch provider blocks with affected logical target and owner action. Runtime execution for AC-005 through AC-008 is covered by provider execution cases, not this dry-run display case. |
| FWK-IT-011 | AC-012, AC-013, AC-016 | Provider evidence normalization | Mock, stub, ephemeral, and native provider results are normalized into run evidence with provider_id, provider_type, profile, runtime_mode, registry status, Provider Contract path, Provider Instance path, resolved operation result, assertion result, cleanup result, release-evidence eligibility, and final status. |
...
| SUP-IT-004 | SUP-AC-004 | F004 | Current framework verification smoke validates that owner-authored RP/RU mapping is explicit, Provider Contracts and Provider Instances are declared, and the framework does not infer generated artifacts. Full Phase 2 Agent Skill validation must additionally emit suite manifest, run plan, Execution Profiles, Provider Instances, Environment Bindings, Provider Contracts, traceability map, and mapping explanation. | EVD-004 |
...
It proves that the framework can validate provider capability registry entries, Provider Contract schemas, Provider Instance shape, Environment Binding values, DSL target resolution, dry-run planning, dispatch, blocking, and normalized evidence for heterogeneous logical execution boundaries. It does not prove a real Product/RP release.

Required interface validation tests:

| Test Area | Required Positive Tests | Required Failure Tests | AC |
|---|---|---|---|
| Provider Contract schema validation | Valid materialized contract files referenced by the provider capability registry define provider_type, allowed operations, allowed `bind_as` values, binding keys, output refs, evidence outputs, failure codes, defaults, `safety.rules`, and valid instance shape | Missing contract file, missing provider_type, duplicate operation, missing allowed `bind_as`, missing output refs, missing required `safety.rules` for command-capable operations, unsafe defaults, unsupported failure code, or registry entry pointing to a nonexistent path blocks before dispatch | AC-016 |
| Provider Instance validation | Valid instances declare provider_id/provider_type and use the same top-level shape as the Provider Contract | Missing instance, unknown field, unknown operation, unsupported `bind_as`, output ref not in contract, or failure code not in contract blocks before dispatch | AC-002 / AC-016 |
| Environment Binding validation | Valid binding supplies all required binding keys for selected profile using refs or non-secret actual values | Missing Environment Binding, missing provider binding, missing binding key, raw secret, or incompatible profile blocks before dispatch | AC-002 / AC-014 / AC-018 |
| Data Binding validation | Valid DSL `data_binding` entries use allowed categories and `${data.<category>.<name>}` refs that resolve to reviewed artifact `ref` values | Unknown category, missing `ref`, unresolved data ref, inline endpoint/JDBC/broker URL, raw secret, or using `data_binding` as an Environment Binding substitute blocks before execution | AC-001 / AC-004 / AC-014 |
| Dependency provisioning validation | Local/CI Execution Profiles declare allowed Testcontainers or equivalent ephemeral dependencies, readiness checks, cleanup scope, and output binding keys | Unsupported provisioner, missing image/catalog ref, missing readiness check, missing cleanup scope, or missing output binding key blocks before dispatch | AC-002 / AC-016 |
| DSL target resolution | DSL target provider_id + profile resolves to Provider Instance, Provider Contract, and Environment Binding | Unknown provider_id, missing profile, mismatched provider_type, invalid operation, invalid `bind_as`, or missing output ref blocks before execution | AC-001 / AC-002 |
...
| Evidence contract | Provider execution writes provider_id, provider_type, profile, operation, resolved operation result, Provider Contract ref, Provider Instance ref, and evidence refs | Missing provider identity, missing resolved operation result, or unmasked secret makes evidence invalid | AC-012 / AC-013 / AC-014 |
...
|---|---|---|---|
| Request/response | Resolve `rest_client`, `grpc_client`, and `wiremock_http_mock` contracts, instances, Environment Bindings, payload `bind_as`, response mapping, timeout, explicit mock/stub endpoint binding for local/CI, WireMock mappings ref, base_url output injection, URI joining, non-2xx status, native gRPC timeout/non-timeout failure, and assertion reference. | Missing base URL/service binding key, missing WireMock mappings ref, unsupported runtime_mode, prohibited SIT/preprod mock endpoint, unsupported operation, invalid request `bind_as`, missing output ref, malformed operation map, provider exception, timeout, or non-success response blocks or fails with normalized evidence before downstream release evidence is claimed. | provider_id, provider_type, profile, runtime_mode, action, request binding ref, response evidence ref, WireMock request journal/server log when selected, HTTP status or gRPC status/error, timeout flag, assertion result |
| Messaging | Resolve `kafka_messaging` and `nats_messaging` Provider Contracts/Instances for `publish_message`, `consume_message`, NATS `request_reply_message`, observe, and cleanup with topic/subject binding keys, fake-topic or embedded-broker binding for local/CI, payload `bind_as`, `event_published`, `event_payload_match`, `consume_from: test_start_time`, correlation id when required, timeout, observation rule, cleanup strategy, action fallback, and max cleanup count. | Missing topic/subject binding key, unsupported runtime_mode, prohibited SIT/preprod fake broker, missing publish/request payload binding, invalid `bind_as`, missing cleanup strategy/count, missing correlation id where required, unsupported mode, unsupported serialization, malformed action shape, transport failure, JetStream-only behavior selected without P1 Provider Contract support, or unsupported provider_type blocks or fails before downstream release evidence is claimed. | provider_id, provider_type, profile, runtime_mode, topic/subject ref, action mode, payload binding, correlation id, observed message/event ref, request/reply output ref, cleanup result, message count, observation window, attempts, timeout/failure classification |
| DB fixture | Resolve `jdbc_database` Provider Contract/Instance and Environment Binding for seed/query/cleanup, `secret_ref` or runtime-generated connection binding key, Oracle / DB2 dialect, SQL params binding, mock DB or disposable schema binding for local/CI, isolation key, cleanup marker, cleanup strategy, verification query rows, and malformed query-row skipping. | Mutating setup without cleanup, missing connection binding key, inline credential, unsupported runtime_mode, prohibited SIT/preprod mock DB, unsupported dialect, unsafe query ref, missing SQL ref, missing SQL file, invalid `bind_as`, invalid params binding, or SQL failure blocks before setup or fails with framework evidence. | provider_id, provider_type, profile, runtime_mode, seed ref, query ref, dialect, masked params, row count, query result ref, cleanup result, postcondition evidence |
| Deployment readiness | Resolve `kubernetes_runtime` and `vm_runtime` contracts, instances, version/deployment refs, readiness probes, bounded K8s direct API readiness, bounded K8s pod log capture, bounded VM SSH/WinRM command probes, blank timeout fallback, native probe IO fallback, and failure-evidence write failure. | Missing namespace/host binding key, API server ref, pod selector/log tail bound, VM command ref, readiness probe, Environment Binding, provider capability, native IO failure, or failed evidence write blocks affected logical target before execution or reports an actionable technical failure. | provider_id, provider_type, profile, readiness status, deployment/version ref, affected target, captured pod log ref or VM command output ref when configured |
| File/batch | Resolve `shell_command` contract/instance with command binding key, required `safety.access_policy`, input refs, output refs, logs, success codes, and timeout | Missing command binding key, missing required safety policy, unsafe command allowlist, invalid `bind_as`, missing output ref, timeout, or non-success exit code produces blocked or failed run evidence | provider_id, provider_type, profile, safety policy status, stdout/stderr refs, actual output ref, exit code, timeout flag |
...
Every provider public-interface case must also verify dry-run output. Dry-run must name provider_id, provider_type, profile, registry status, capability, affected logical target, Provider Contract path, Provider Instance path, Environment Binding path, binding key status, AP gate, and owner action for missing, ambiguous, unsupported, invalid `bind_as`, missing output ref, missing binding key, or unapproved escape-hatch capabilities.
...
This is a pre-provider-runtime gate. FWK-008 must be green before sample fixture migration, provider dispatch expansion, or native runtime work can claim support for the new execution-focused DSL shape. FWK-008 includes parser validation, profile/binding validation, secret guardrails, plugin catalog validation, result schema validation, and one CLI run/report proof that the same v0.2 artifact is usable by execution and reporting.

| DSL Area | Positive Verification | Failure Verification |
|---|---|---|
| Identity and source refs | Reads `dsl_version: v0.2`, `test_case_id`, `status`, `revision`, tags, `source_refs.acceptance_criteria`, optional `labels`, and optional `compatible_profiles` | Missing acceptance-criteria source ref, unsupported `status`, incompatible selected profile, or label-driven runtime branching blocks before execution. |
| Targets | Resolves multiple named `targets` with `provider_id` and `profile`, then resolves Provider Instance, Provider Contract, and Environment Binding | Missing target, unknown provider_id, missing profile, missing Provider Instance, missing Provider Contract, or missing Environment Binding blocks before provider dispatch. |
| Parameters | Resolves operation-level `parameters[].ref` into reviewed values, validates `parameters[].bind_as` against the selected Provider Contract, and creates separate run IDs, result JSON, and run evidence with `parameter_case_id` where parameter sets contain multiple cases | Missing or unreadable parameter ref, invalid `bind_as`, duplicate case ID in the referenced set, missing values, or unresolved parameter reference blocks before provider dispatch. |
| Setup | Resolves `setup.fixtures`, `cleanup_ref`, and `${data.<category>.<name>}` references for state-mutating fixtures | State-changing fixture without cleanup reference, unknown `data_binding` category, missing data artifact `ref`, unresolved data ref, or raw runtime value in `data_binding` reports a fixture/schema-policy gap. |
| Execute | Resolves v0.2 `execute[].operation`, `target`, `parameters[].ref`, `parameters[].bind_as`, and `outputs` for one or more uniquely named steps | Duplicate step IDs, ambiguous ordering, `call_ru`, `target_ru_id`, missing outputs, missing output refs in the Provider Contract, invalid `bind_as`, or unsupported operation blocks before execution. |
...
- Valid execution-focused DSL with multiple targets, setup fixture, parameter ref, one or more execute steps, captured outputs, expected result, verify rules, evidence refs, timeout, and retry passes validation.
...
- Duplicate or ambiguous executable `execute[]` steps block before provider dispatch so the runtime cannot silently execute only the primary step.
...
- Operation-level `parameters[].ref` with a reviewed two-case parameter set and `parameters[].bind_as` creates two run IDs, two run evidence directories, recorded `parameter_case_id`, resolved parameter values, and batch/report coverage that counts the AC once.
- Malformed v0.2 parameter sets or `bind_as` values not allowed by the selected Provider Contract block before provider dispatch.
...
- CLI dry-run blocks selected tests whose `compatible_profiles` do not include the requested `--env` profile; generated Environment Bindings block when selected provider bindings omit required binding keys; generated Execution Profiles block when required fields are missing or `execution_mode` is unsupported; `sit` and `preprod` profile execution blocks when target readiness refs are missing.
...
|---|---|
| Provider Contract schema validation | Valid contracts define provider_type, allowed operations, allowed `bind_as`, binding keys, output refs, evidence outputs, failure codes, defaults, safety rules, and valid Provider Instance shape; invalid contracts fail deterministically. |
| Provider Instance validation | Valid instances use Provider Contract shape; unknown fields, unsupported operations, invalid `bind_as`, unsupported output refs, or unsupported failure codes block before dispatch. |
| Environment Binding validation | Selected profile binding supplies all required binding keys using refs or non-secret actual values; missing binding, missing key, raw secret, or incompatible runtime mode blocks. |
| DSL target resolution | DSL `provider_id` + `profile` resolves to Provider Instance, Provider Contract, and Environment Binding; missing or ambiguous resolution blocks. |
| Invalid `bind_as` | Operation-level `parameters[].bind_as` must be allowed by the referenced Provider Contract operation. |
| Missing binding key | Required binding keys in Provider Contract and Provider Instance must be present in Environment Binding. |
| Missing Provider Contract | Provider Instance `provider_type` without a materialized Provider Contract blocks validation and dry-run. |
| Missing Provider Instance | DSL target `provider_id` without a matching Provider Instance blocks validation and dry-run. |
| Missing Environment Binding | Selected profile without provider binding blocks validation and dry-run. |
...
| Verification | Required Coverage |
|---|---|
| Public CLI validate | `regress validate --suite samples/golden_e2e/suite_manifest.yaml` loads the suite and validates DSL, Provider Contract, Provider Instance, Execution Profile, Environment Binding, target resolution, expected result refs, and secret guardrails. |
...
| Failure paths | Invalid contract/instance/binding, unsupported operation, unsupported `bind_as`, missing output ref, missing expected artifact, raw secret, provider-specific mismatch/timeout, and required evidence missing. |
...
| FWK-004 | AC-001, AC-002, AC-003, AC-004, AC-005, AC-006, AC-007, AC-008, AC-009, AC-010, AC-011, AC-012, AC-013, AC-014, AC-016 | DSL, Execution Profile, Environment Binding, Provider Instance, Provider Contract, fixture, parameter, expected-result, secret, or verify-rule gaps block or fail with actionable evidence | `./mvnw verify` | P1 | Auto |
...
| FWK-007 | AC-002, AC-004, AC-014, AC-016 | Provider negative cases block before unsafe execution and report provider_id, provider_type, profile, registry status, escape-hatch approval status when applicable, capability, affected logical target, Provider Contract path, Provider Instance path, Environment Binding path, AP gate, and owner action | `./mvnw verify` | P1 | Auto |
| FWK-008 | AC-001, AC-002, AC-003, AC-012, AC-013, AC-014, AC-015, AC-017, AC-018 | Execution-focused DSL v0.2 parser/resolver verifies source refs, labels, profiles, operation parameters, suite/test/tag selection, generated Execution Profile fields, execution-mode enum, Provider Instance refs, Environment Binding refs, SIT/preprod readiness refs, evidence, runtime, result schema, secret blocking, and report consumption. | `./mvnw test` | P1 | Auto |
...
| FWK-010 | AC-005, AC-006, AC-007, AC-008, AC-016 | Provider type catalog verification covers shell command, REST, gRPC, WireMock HTTP mock, JDBC, NATS, Kafka, K8s runtime, VM runtime, external runner, supported provider modes, registry metadata, Provider Contract/Instance compatibility such as numeric-string cleanup bounds and canonical messaging operations `publish_message`, `consume_message`, and `request_reply_message`, and unsupported provider blocking | `./mvnw verify` | P1 | Auto |
...
| FWK-012 | AC-004, AC-012, AC-013, AC-014 | Fixture/evidence/result verification covers `data_binding` categories, data refs, prohibited raw runtime values, `db_seed`, `db_cleanup`, `http_stub`, setup, cleanup precedence across DSL policy, Provider Contract strategy, Provider Instance selection, Execution Profile restrictions, masking, evidence indexing, result schema, coverage denominator rules for automatable and partial AC, exclusion metadata gaps, and failure classification | `./mvnw verify` | P1 | Auto |
| FWK-013 | AC-001, AC-002, AC-012, AC-013, AC-015, AC-016 | Documented v0.2 public interface verification covers the user guide, CLI invocation, DSL/test definition fields, Execution Profile fields, Provider Instance fields, Provider Contract fields, Environment Binding fields, result/evidence schemas, stable output keys, artifact locations, and next-stage support command boundary with controlled breaking changes allowed before v1.0 | Docs review plus contract tests | P1 | Auto / Agent-assisted |
...
- `regress validate` supports Product Repo mode with `--rp-id` and `--env`, and framework sample mode with `--suite <suite_manifest_path>`. It supports optional `--root` default `.`, `--test-case`, `--tag`, `--format yaml|json`, and `--strict`, returns exit code `0`, `1`, or `2`, and prints stable keys `status`, `valid`, `errors`, `warnings`, `selected_tests`, `provider_instances_used`, `provider_contracts_used`, `environment_bindings_used`, and `owner_action`.
- `regress run` supports Product Repo mode with `--rp-id` and `--env`, and framework sample mode with `--suite <suite_manifest_path>` and `--profile <profile>`. It supports optional `--root` default `.`, `--dry-run`, `--test-case`, and `--tag`, creates batch/run evidence when execution starts, and prints stable dry-run and run keys.
...
- DSL and suite contracts exist and define stable runtime-consumed fields for identity/status/source refs, labels, compatible profiles, operation parameters, targets, setup, execute, cleanup, expected results, verify, evidence, runtime policy, and suite/test/tag selection.
- Execution Profile and Environment Binding contracts exist and define stable runtime-consumed fields for profile selection, execution mode, target provider bindings, secret refs, readiness refs, dependency model, constraints, and binding key values.
- Provider Contract and Provider Instance contracts exist and define stable fields for provider_id, provider_type, required binding keys, allowed operations, allowed `bind_as` values, supported execution modes, runtime status, cleanup strategy, output refs, evidence outputs, safety constraints, and failure codes.
...
- `./mvnw test` passes.
- `./mvnw verify` passes.
- `./mvnw -Pv02-delivery-coverage verify` passes before v0.2 delivery acceptance; this profile applies class-level LINE 1.00 and BRANCH 0.90 to critical framework public-interface and runtime classes.
...
- `FWK-001` through `FWK-013` plus `FWK-015` and `FWK-016` are either automated or explicitly mapped to an automated test class/method. `FWK-014` remains a later broader heterogeneous runtime scenario beyond Track C P0.
...
| `execution_profile.v0.2.schema.yaml` | FWK-002 / FWK-004 / FWK-008 / FWK-013 |
| `environment_binding.v0.2.schema.yaml` | FWK-002 / FWK-004 / FWK-007 / FWK-008 / FWK-013 |
...
| AC-006 | FWK-006 / FWK-010 / FWK-016 | FWK-007 / FWK-010 / FWK-016 | FWK-010 |
...
| AC-012 | FWK-002 / FWK-012 / FWK-013 / FWK-015 / FWK-016 | FWK-004 / FWK-012 / FWK-015 / FWK-016 | FWK-003 / FWK-012 / FWK-013 |
| AC-013 | FWK-002 / FWK-012 / FWK-013 / FWK-015 / FWK-016 | FWK-004 / FWK-012 / FWK-015 / FWK-016 | FWK-003 / FWK-008 / FWK-013 |
| AC-014 | FWK-001 / FWK-012 / FWK-015 / FWK-016 | FWK-004 / FWK-008 / FWK-015 / FWK-016 | FWK-012 |
| AC-015 | FWK-002 / FWK-008 / FWK-013 / FWK-015 / FWK-016 | FWK-005 / FWK-008 / FWK-013 / FWK-015 / FWK-016 | FWK-008 / FWK-013 |
| AC-016 | FWK-006 / FWK-010 / FWK-011 / FWK-013 / FWK-015 / FWK-016 | FWK-003 / FWK-007 / FWK-010 / FWK-015 / FWK-016 | FWK-009 / FWK-010 / FWK-013 |
...
| REST/gRPC request/response provider | `RegressionCommandTest`, `ProviderCapabilityRegistryTest`, `RequestResponseProviderTest`, and `DefaultGrpcClientInvokerTest` request/response cases | Partial framework verification support for REST and native descriptor-driven gRPC unary calls with endpoint/service refs, actions, payload binding, timeout, output ref, and evidence | Pilot endpoint validation remains target provider work. |
...
| Messaging provider | `RegressionCommandTest`, `ProviderCapabilityRegistryTest`, `MessagingProviderTest`, `DefaultMessagingTransportTest`, `ExecutionEngineTest` messaging cases, and `HeterogeneousProviderRuntimeIT` public-run evidence | Partial framework verification support for local/mock messaging and native Kafka/NATS publish, NATS request/reply, consume/observe, and bounded cleanup drain runtime | Real pilot broker validation remains target provider work; Kafka request/reply and persistent broker purge are future work only if the selected RP requires them. |
...
| Heterogeneous pilot validation | Not covered by framework Maven tests | Pending owner-provided RP plus generated artifacts | Requires real `package.yaml`, `rp_feature_spec.md`, `acceptance_criteria.md`, `rp_ru_mapping.yaml`, approved tests, approved truth sources, generated suite/run/Execution Profile/Provider Instance/Provider Contract/Environment Binding artifacts. |

## 7.8 Selected Heterogeneous Pilot Validation Gate

After owner-provided pilot RP artifacts exist, the downstream RP release pipeline shall run Agent Skill translation, then validate the real heterogeneous RP using generated framework artifacts and selected Provider Contracts / Provider Instances / Environment Bindings.
...
- Batch/run evidence includes provider_id, provider_type, profile, registry status, Provider Contract path, Provider Instance path, Environment Binding path, resolved bindings, resolved operation result, assertion result, cleanup result, and final pass/fail status.
- Dry-run blocks missing, ambiguous, unsupported, invalid `bind_as`, missing output ref, missing binding key, or unapproved escape-hatch selected provider types before unsafe execution.

This gate is not satisfied by the sample framework fixture. It requires owner-authored `package.yaml`, `rp_feature_spec.md`, `acceptance_criteria.md`, `rp_ru_mapping.yaml`, approved DSL tests, approved truth sources, generated suite/run/Execution Profile/Provider Instance/Provider Contract/Environment Binding artifacts, and environment readiness records for the pilot RP.
...
That flow produces real RP batch/run evidence under the Product Repo. It may use `local`, `ci`, `sit`, or `preprod` depending on the generated Execution Profile. SIT/preprod runs require deployed target versions and environment readiness evidence.
...
| Provider public-interface contract verification | `./mvnw verify` | Local/CI mock-stub-ephemeral Provider Contract, Provider Instance, and Environment Binding proof for heterogeneous execution boundaries without real downstream RP release evidence. |
| Packaged CLI smoke | `java -jar target/spec-driven-auto-regression-0.1.0-SNAPSHOT.jar validate --root . --rp-id RP-FWK-SAMPLE --env ci` | Verify packaged command delegation through the public runtime command surface. |
