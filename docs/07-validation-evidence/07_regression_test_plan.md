...
| Verify | Supports explicit actual/expected checks plus canonical `selector`, `db_record_exists`, and `event_published` semantics. `json_path_equals` and `json_path_absent` must keep `actual` as the captured output ref and use `selector` for the JSON/YAML path. | Missing `actual`/`expected`, missing selector for JSON path verification, query ref, event ref, or unsupported verify type blocks with verify ID. |
...
| Response assertion provider | `ExecutionEngineTest` response assertion cases, `RequestResponseProviderTest` HTTP evidence cases, `AssertionEngineTest` schema/contract assertion cases, and DSL validator selector cases | Supported for HTTP/status field checks, JSON path equality and absence assertions with canonical `selector`, numeric tolerance assertions, schema/contract assertions, multi-assertion aggregation, and durable assertion evidence over provider actual-output JSON/YAML plus request-response evidence | Add invariant or custom comparator assertions only if selected by the pilot. |
