...
| Messaging | Resolve publish/consume/observe contract with topic or subject ref, payload binding for publish, correlation id when required, timeout, and observation rule | Missing topic/subject ref, missing publish payload binding, missing correlation id where required, unsupported mode, or unsupported serialization blocks before dispatch | Provider family, topic/subject ref, action mode, correlation id, observed message/event ref |
...
| Messaging provider | `RegressionCommandTest`, `ProviderCapabilityRegistryTest`, `MessagingProviderTest`, `DefaultMessagingTransportTest`, and `ExecutionEngineTest` messaging cases | Supported for local/mock messaging and native Kafka/NATS publish plus consume/observe runtime with connection refs, topic/subject refs, publish payload binding, action mode, timeout, output ref, correlation checks, transport dispatch, and evidence normalization | Message cleanup and real pilot broker validation remain target provider work. |
