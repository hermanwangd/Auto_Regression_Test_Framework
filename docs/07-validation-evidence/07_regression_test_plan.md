...
| Packaged CLI smoke verification | Spring Boot jar CLI entrypoint | `java -jar target/spec-driven-auto-regression-0.2.1.jar validate --root . --rp-id RP-FWK-SAMPLE --env ci` | Sample Product Repo fixture or current repository fixture | CLI output and exit code. |
...
| Packaged CLI smoke | `java -jar target/spec-driven-auto-regression-0.2.1.jar validate --root . --rp-id RP-FWK-SAMPLE --env ci` | Verify packaged command delegation through the public runtime command surface. |
