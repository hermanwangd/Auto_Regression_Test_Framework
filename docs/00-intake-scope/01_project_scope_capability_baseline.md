...
| CAP-007 | Provider Interface and Catalog | Provider types such as `shell_command`, `rest_client`, `grpc_client`, `jdbc`, `nats`, `kafka_messaging`, `kubernetes_runtime`, `vm_runtime`, and approved `external_runner` Provider Contracts | Platform | Provider catalog and plugin metadata validation |
