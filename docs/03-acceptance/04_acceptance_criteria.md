...
Boundary path: This AC covers client providers only. Kafka and IBM MQ v0.2 provider contracts may define native/mock/ephemeral runtime vocabulary, but only `mock` is executable in this framework build. Kafka broker provisioning, IBM MQ queue-manager provisioning, Testcontainer startup, RU startup, broker administrator purge, shared consumer offset mutation, destructive IBM MQ `mq_get`, Kafka request/reply, and advanced NATS JetStream stream/consumer handling are not default capabilities and must fail as unsupported unless a selected future slice adds an approved reusable Provider Contract and executable runtime mode.
