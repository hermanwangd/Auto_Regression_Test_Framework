...
Failure path: Given missing endpoint binding, missing WireMock mappings ref, unsupported method, timeout, invalid payload binding, invalid operation input key, missing request journal output ref, or prohibited SIT/preprod mock substitution, execution blocks or fails with actionable details.
...
- Invalid operation input key, missing required input, missing Provider Instance, unknown provider type or missing explicit custom Provider Contract, missing Environment Binding, missing binding key, and missing output ref fail before provider dispatch with taxonomy-backed errors.
