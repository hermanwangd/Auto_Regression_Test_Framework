...
| AC-006 | Execute HTTP, SOAP mock, gRPC mock, and request/response targets |
...
Happy path: Given `generated-framework/run_plan.yaml` with an Env_Profile ref, matching `generated-framework/env_profiles/<env_profile_id>.yaml`, generated Provider Instances, and the framework Provider Contract catalog, the framework resolves env_profile_id, supported execution mode (`local`, `ci`, `sit`, or `preprod`), default-backed isolation scope, dependency model, dependency substitution policy, dependency provisioning policy, data policy, max duration, logical targets, target dependencies, `provider_id`, Provider Instance, `provider_type`, built-in Provider Contract, selected `runtime_mode`, required binding keys, Env_Profile provider binding values, and valid `generated_ref` targets from producing Provider Contract `bindable_outputs` or declared dependency provisioner outputs. Only `env_profile_id`, `execution_mode`, and `providers` are always required in Env_Profile; policy sections are required only when the profile needs to override framework defaults.

Failure path: Given a missing Env_Profile ref, missing referenced Env_Profile file, missing core Env_Profile field, unsupported execution mode, unsupported provisioner when provisioning policy is declared, missing dependency definition when a generated dependency is referenced, missing readiness check when readiness evidence is required, missing cleanup scope when explicitly required, unsupported or prohibited `runtime_mode`, a `runtime_mode` not listed by Provider Contract `executable_runtime_modes` when present, incompatible Env_Profile, unresolved target, missing Provider Instance, unknown `provider_type` in the framework Provider Contract catalog, missing explicit custom Provider Contract, Provider Instance shape mismatch, invalid operation input key, missing output ref, missing Env_Profile provider entry, missing required binding key, unknown binding key, invalid binding value kind, invalid enum value, invalid `generated_ref`, missing bindable output, undeclared mock/stub/ephemeral replacement, or prohibited destructive operation, execution blocks before provider dispatch with `target_resolution_error` or `environment_error`.

Boundary path: Given `local` or `ci` execution, external service, DB, messaging, K8s, or VM dependencies may be replaced only by explicitly declared mock, stub, ephemeral, fake-topic, embedded-broker, disposable-schema, generated-data, or Testcontainers-backed Env_Profile provider binding values allowed by the selected Env_Profile, Provider Contract, and Provider Instance. Generated dependency values must be referenced by Env_Profile binding keys using `generated_ref` and must target Provider Contract `bindable_outputs` or declared dependency provisioner outputs. Given `sit` or `preprod` execution, the selected Env_Profile must reference deployed runtime readiness, must default to `runtime_mode: native`, and must not use mock substitution for release evidence.
...
Happy path: Given a `rest_client` Provider Instance, selected profile, resolved `base_url`, and allowed `http_request` operation, the framework sends the configured request, captures `response.status`, `response.headers`, `response.body`, `response.duration_ms`, and `http_request_response` evidence. Given a local/CI `wiremock_http_mock` Provider Instance, the framework loads checked-in stub mappings, exposes generated `base_url` for the HTTP client target, records request journal/server log evidence, and verifies `http_mock_called`, `http_mock_request_body_match`, or response assertions. Given a local/CI `soap_mock` Provider Instance, the framework uses WireMock-backed SOAP stubs to expose generated `endpoint_url`, match SOAPAction/header and XPath/XML request conditions, return checked-in XML response fixtures, record request journal/server log evidence, and verify `soap_request_received`. Given a local/CI `grpc_mock` Provider Instance, the framework uses the WireMock gRPC extension with descriptor/proto refs to expose generated `target_uri`, load unary request/response stubs, verify `grpc_request_received`, and record request journal/server log evidence. Given a `grpc_client` Provider Instance and allowed `unary_call`, the framework captures gRPC status, metadata, body refs, timing, and `grpc_request_response` evidence where the selected runtime is implemented.

Failure path: Given missing endpoint binding, missing generated WireMock `base_url`, missing SOAP `endpoint_url`, missing gRPC `target_uri`, missing WireMock mappings ref, missing SOAP response ref, missing gRPC descriptor ref, unsupported HTTP method, unsupported SOAP/gRPC operation, timeout, invalid payload binding, invalid XPath, invalid proto descriptor, invalid operation input key, missing response output ref, missing request journal output ref, assertion mismatch, non-success provider response where success is required, or prohibited SIT/preprod mock substitution, execution blocks or fails with actionable details and preserves provider failure evidence.

Boundary path: Empty request bodies and no-content responses such as HTTP 204 are valid when explicitly represented by checked-in refs and expected outputs. SOAP empty body, SOAP fault, and XML namespace matching must be explicit in checked-in refs or XPath expressions. gRPC v0.2 mock scope is unary only; client/server streaming must fail as unsupported unless a future contract adds it. Raw URLs and credentials belong in Env_Profile binding keys or secret refs, not in the DSL body. WireMock-backed mock evidence is valid for local/CI framework provider capability verification and isolated dependency replacement; it must not replace internal RP/RU dependencies or claim SIT/preprod release evidence.
...
### WireMock-backed SOAP and gRPC Mock Samples

PR-008 SOAP/gRPC mock samples are accepted only after checked-in samples validate, dry-run, execute, and report:

- SOAP happy path: start or connect WireMock-backed SOAP mock, load checked-in SOAP XML response, match SOAPAction/header and XPath request condition, return XML response, record request journal/server log/HTTP response/assertion evidence, and report consumption.
- SOAP failure path: missing SOAP request, XPath mismatch, or XML response mismatch produces owner-actionable assertion/failure evidence.
- SOAP boundary path: SOAP fault and namespace-aware XPath matching are handled explicitly without treating namespace prefixes as business truth.
- gRPC happy path: start or connect WireMock gRPC-extension mock, load descriptor-backed unary stub, match request JSON, return response fixture, record request journal/server log/protobuf JSON diff evidence, and report consumption.
- gRPC failure path: missing descriptor, unknown service/method, request mismatch, or non-OK gRPC status produces owner-actionable failure evidence.
- gRPC boundary path: unary-only scope is enforced; streaming calls fail before execution as unsupported.
...
## AC-008 Observe Messaging Events and Queue Messages

Happy path: Given `nats`, `kafka`, or `ibm_mq` Provider Instances with Env_Profile binding values, the framework can publish/put, observe/browse, or match declared messages and retain masked message evidence. Given Kafka `kafka_publish`, `kafka_observe`, or `kafka_payload_match`, the framework uses Env_Profile `bootstrap_servers`, `topic`, and `consumer_group`, observes from `consume_from: test_start_time` by default when possible, and records topic, partition, offset, key, attempts, observation window, timeout status, and payload-match evidence. Given IBM MQ `mq_put`, `mq_browse`, `mq_message_exists`, or `mq_payload_match`, the framework uses Env_Profile queue-manager, channel, connection, queue, and credential refs, browses shared queues by default, and records queue, message_id, correlation_id, attempts, observation window, timeout status, and payload-match evidence. Given NATS `nats_publish`, `nats_observe`, `event_published`, or `event_payload_match`, the framework observes from `consume_from: test_start_time` by default when possible, handles subjects, captures matched fields, attempts, observation window, and timeout status. Kafka topic/consumer group and IBM MQ queue are resolved from Env_Profile binding keys; multiple destinations require multiple Provider Instances. A single messaging suite may run Kafka and IBM MQ test cases under one selected Env_Profile and must report suite-level `test_count`, per-test status, provider evidence summary, and result JSON across both provider types.

Failure path: Given missing Kafka topic, missing Kafka consumer group, missing IBM MQ queue manager, missing queue, missing credential secret_ref, missing topic/subject ref, operation-level Kafka topic/consumer_group override, operation-level IBM MQ queue override, missing payload binding, invalid ref path, invalid timeout, unsupported provider mode, unsupported destructive MQ operation, connection error, broker timeout, or payload mismatch, the run blocks or fails with owner-actionable output and normalized evidence when execution has started.

Boundary path: This AC covers client providers only. Kafka and IBM MQ v0.2 provider contracts may define native/mock/ephemeral runtime vocabulary; this framework build executes `mock` plus externally supplied `native` endpoints and treats `ephemeral` as non-executable in v0.2.7. Kafka broker provisioning, IBM MQ queue-manager provisioning, Testcontainer startup, RU startup, broker administrator purge, shared consumer offset mutation, destructive IBM MQ `mq_get`, Kafka request/reply, Kafka SASL/TLS/schema-registry modes, IBM MQ CCDT/TLS/message-selector modes, and advanced NATS JetStream stream/consumer handling are not default capabilities and must fail as unsupported unless a selected future slice adds an approved reusable Provider Contract and executable runtime mode.
...
Happy path: Each suite execution produces one standard result JSON with framework version, DSL version, suite/run identifiers when execution starts, test case ID or suite-level multi-test ID, status, profile, environment, timestamps, labels, steps, provider_id, provider_type, resolved operation result, verify results, evidence refs, and failure object. Multi-test suite results include `test_count` and `test_results[]`.

Failure path: If result JSON cannot be written or does not satisfy `result.v0.2.schema.yaml`, the run is not complete. If a reported result declares `test_count` or `test_results`, then missing paired fields, non-integer `test_count`, mismatched `test_count` versus `test_results.length`, a non-object `test_results[]` entry, a per-test result missing `test_case_id`, `status`, or `profile`, or a per-test status outside `passed`, `failed`, and `blocked` makes `regress report --result` or `regress validate-evidence --result` invalid.

Boundary path: Technical failure classification is framework-owned; product defect or spec ambiguity classification belongs to Agent Skill or later triage. Legacy single-test contract-baseline samples without `test_count/test_results` remain accepted only for compatibility.
...
Happy path: CLI supports the invocation portion of the documented v0.2 public interface with controlled breaking changes allowed before v1.0: `regress validate --rp-id <rp-id> --env <env_profile_id>`, `regress run --rp-id <rp-id> --env <env_profile_id>` with optional `--dry-run`, `--test-case <test-case-id>`, `--suite <suite-id>`, or `--tag <tag>`, and `regress report --rp-id <rp-id> --batch-id <batch-id>`. `--root <product-repo>` is optional and defaults to the current directory. Framework sample mode supports suite manifests with `tests[]` sharing one selected suite-level profile, plus compatibility `child_suites[]` aggregation manifests. A standard suite run returns one suite-level `batch_id`, one `run_id`, `test_count`, `test_results[]`, result JSON, and evidence directory for all selected tests. `validate` produces deterministic schema, DSL, Provider Contract, Provider Instance, Env_Profile, result/evidence, child suite ref, duplicate child id, and secret guardrail findings without execution. `run --dry-run` produces a resolved execution plan or child-suite aggregation plan containing provider_id, Provider Instance, provider_type, Provider Contract, env_profile_id, Env_Profile provider binding, required binding key status, bindable output ref status, output refs, child suites when applicable, and blocked owner actions without executing real provider operations.
...
- P1 Kafka and IBM MQ client-provider contracts validate provider instances, Env_Profile bindings, dry-run resolution, safe non-mutating observation defaults, and messaging evidence shape before native broker execution is claimed.
