...
Happy path: Given a valid `dsl_version: v0.2` test with required metadata, source refs, targets using `provider_id` and `profile`, setup/execute/cleanup operations, operation parameters using `ref` and `bind_as`, verify, evidence, and runtime, validation passes and reports the consuming AP for each section.

Failure path: Given missing required fields, duplicate execute/verify IDs, unsupported enum values, governance fields, legacy-only fields in a new artifact, a target without `provider_id` or `profile`, an unresolved `${data.<category>.<name>}` reference, prohibited raw runtime values in `data_binding`, an operation not allowed by the referenced Provider Contract, an invalid `bind_as`, or an output ref not declared by the Provider Contract, validation blocks before execution with `schema_error` or `target_resolution_error`.
...
Happy path: Given `nats_messaging` or `kafka_messaging` Provider Instances with Environment Binding values, the framework can publish, consume, or observe declared events through `publish_message` or `consume_message` and retain event payload evidence. Given NATS `event_published` or `event_payload_match`, the framework observes from `consume_from: test_start_time` by default when possible, handles subjects, captures matched fields, attempts, observation window, and timeout status. Given NATS request/reply declared through `request_reply_message` with `mode: request_reply`, the framework sends the request payload, captures reply output ref, correlation when required, timeout, and request/reply evidence.

Failure path: Given missing topic/subject ref, missing payload binding, unsupported serialization, missing correlation when required, unsupported provider mode, missing reply output ref, or broker timeout, the run blocks or fails with owner action.
...
Happy path: Provider Contracts are materialized files referenced by the provider capability registry and define provider_type, allowed runtime modes, allowed operations, allowed `bind_as` values, binding keys, output refs, evidence outputs, failure codes, defaults, and valid Provider Instance shape. Provider Instances declare provider_id, provider_type, allowed runtime modes, and conform to the referenced Provider Contract. Environment Bindings supply `runtime_mode` and all required binding keys for the selected profile. Provider and verify plugins expose metadata for ID, version, provider_type, supported operations or verify types, required binding keys, supported execution modes, runtime status, evidence outputs, and safety constraints; framework validation checks the catalog before execution. The provider catalog includes local/CI HTTP mock support through `wiremock_http_mock`, JDBC support through `jdbc_database`, NATS support through `nats_messaging`, artifact comparison support through `artifact_compare`, and polling support through `polling_observer` only when the framework can validate and execute the capability or block it with a precise unsupported-capability finding.

Failure path: Missing plugin metadata, unsupported version, ambiguous plugin ID, unsupported provider_type, unsupported `runtime_mode`, missing Provider Contract, missing Provider Instance, missing Environment Binding, Provider Instance field not allowed by its Provider Contract, invalid `bind_as`, missing output ref, missing required binding key, missing required `safety.access_policy`, unsafe command-capable provider policy, undeclared mock substitution, unsupported capability, prohibited mock use in `sit` or `preprod`, or unapproved escape-hatch provider blocks before execution.
...
- DSL targets resolve through `provider_id` and `profile`; Provider Instance, Provider Contract, and Environment Binding resolution rules are specified.
...
- Invalid `bind_as`, missing Provider Instance, missing Provider Contract, missing Environment Binding, missing binding key, and missing output ref fail before provider dispatch with taxonomy-backed errors.
...
- Sample artifacts exist for suite manifest, DSL test case, Provider Contracts, Provider Instances, Execution Profiles, Environment Bindings, expected result, fixtures, result JSON, and evidence index.
...
- DSL, Provider Contract, Provider Instance, Execution Profile, Environment Binding, target resolution, and secret guardrails are validated before execution.
...
Happy path: Given owner-authored `rp_ru_mapping.yaml`, release/deployment context, selected validation boundary, and required provider types, the Agent Skill emits `suite_manifest.yaml`, `run_plan.yaml`, Execution Profiles, Provider Instances, Environment Bindings, Provider Contracts, `traceability_map.yaml`, and a mapping explanation with source facts and unresolved assumptions.

Failure path: Given missing RP/RU membership, ambiguous target mapping, missing Environment Binding, missing Provider Instance, missing Provider Contract, unsupported provider_type, or unresolved dependency graph, translation blocks with owner action and does not emit execution-ready artifacts.
