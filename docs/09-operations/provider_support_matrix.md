...
This matrix defines the Framework `0.3.0` public provider support claim. It does not decide downstream Product/RP release readiness; owners still provide AC, Env_Profile, deployment readiness, expected results, and release evidence.
...
| `common_verify` | `supported` | `samples/20-provider-capability-p0/verification/common_verify/` | JSON/schema/file assertion-only verifier contract. |
...
| `http_mock` | `supported` | `samples/20-provider-capability-p0/http/rest_client_with_wiremock/` | v0.3 protocol-level HTTP mock runtime backed by WireMock internals. |
| `ibm_mq` | `supported` | `samples/20-provider-capability-p0/messaging/ibm_mq/` | Local mock/client sample is release-verifiable. Native external execution runs only when queue-manager bindings are configured; the framework does not start IBM MQ. |
| `jdbc` | `supported` | `samples/20-provider-capability-p0/data/jdbc/` | JDBC fixture/query/cleanup with local H2 Oracle-compatible mode plus external `env://JDBC_CONNECTION` profile validation path. |
| `kafka` | `supported` | `samples/20-provider-capability-p0/messaging/kafka/` | Local mock/client sample is release-verifiable. Native external execution runs only when broker bindings are configured; the framework does not start Kafka. |
...
| `rest_client` | `supported` | `samples/20-provider-capability-p0/http/rest_client_with_wiremock/` and `samples/20-provider-capability-p0/rpc/soap_mock/` | HTTP request client provider with Env_Profile-supplied endpoints or generated mock endpoints. |
