...
This matrix defines the Framework `0.2.5` public provider support claim. It does not decide downstream Product/RP release readiness; owners still provide AC, Env_Profile, deployment readiness, expected results, and release evidence.
...
| `vm_runtime` | `contract_only` | n/a | Contract vocabulary is documented; no release-verifiable runtime sample is published in v0.2.5. |
...
Framework `0.2.5` supports only explicit mixed-provider suite paths:
