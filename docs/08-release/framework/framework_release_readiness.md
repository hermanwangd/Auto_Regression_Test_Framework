...
- Maven artifact version must be immutable, for example `0.2.0`, and must not contain `SNAPSHOT`.
...
- Git tags must use `v<artifact-version>`, for example `v0.2.0`.
...
- `checksums.sha256`
- signature and certificate files for the jar, SBOMs, and checksum file
